# THIS PART OF VANISH IS GRABBED FROM HELIUM!
import websocket, threading, random, json, time, os, requests; from colorama import Fore
import os, requests, tls_client, datetime, sys, hashlib, threading, random, json, time, websocket, httpx
...
def scraper(guildid, channelid):
    with open("data/ids.txt", 'w+') as f: f.truncate(0)
    token = ""
    tokensinguild = []
    ...
    try:
        token = random.choice(tokensinguild)
         ...
         print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->    {r}Failed no tokens in Guild {c}{token}{g}****{r}  {g}{rr.text}{r}")
         exit(0)
    members = scrape(token, f"{guildid}", f"{channelid}")
    for member in members:
        open("data/ids.txt",  "a+").write(str(member) + "\n")

def selfscraper(token, guildid, channelid):
    with open("data/members.txt", 'w+') as f: f.truncate(0)
    url = f"https://discord.com/api/v9/guilds/{guildid}"
    try:
        rr = requests.get(url, headers={"authorization":token})
        if rr.status_code == 200:
            print(f"  {datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SCRAPPER]   {g}->   {r}Scrapping..")
    except:
        print(f"  {datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SCRAPPER]   {g}->   {r}Join the server first..")
        exit(0)
    members = scrape(token, f"{guildid}", f"{channelid}")
    for member in members:
        print(f"  {datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[SCRAPPER]   {g}->   {r}Scrapped!")
        open("data/members.txt",  "a+").write(str(member) + "\n")

# WHY YOU LOOK HERE?
# WTF.. NICE DEOBF BRATKU
# JD (JEBAC DRYBLASA)
